...
for (const name of listTools) {
  ...
  console.log(`${name}: v${version} current ${current} bytes`);
...
console.log(`directResult: v${version} current ${directResult} bytes`);
...
console.log(`directGraphql: v${version} current ${directGraphql} bytes`);
...
console.log(`directBatch: v${version} current ${directBatch} bytes`);
...
for (const [name, additions] of scenarios) {
  ...
  console.log(`${name}: v0.5 baseline ${expected.baseline} bytes; v${version} current ${current} bytes`);
