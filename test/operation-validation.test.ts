...
describe('operation validation messages', () => {
  ...
  it('preserves semantic failure messages on the named-request and batch paths', async () => {
    ...
    expect(namedValidationMessage('list_issues', variables)).toMatchInlineSnapshot(
      `"Invalid parameters for "list_issues": team is required when state is a name. For cross-team calls, use { "assignee": "me", "stateType": "started" }. Valid parameters: canonical fields issues, query, team, state, stateType, assignee, project, sort, after, before, first, last, includeArchived, orderBy, filter, view. Example: { "operation": "list_issues", "variables": { "assignee": "me", "stateType": "started" } }."`,
    );
